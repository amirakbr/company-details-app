function LeftSide({companylist , HandleEdit , HandleDelete}) {
     let listShow = companylist.map((index,id)=> index.Name.length >= 1 ? <tr key={id} id={id} className="border-t-2 
     ...
     <td className="flex gap-6 items-center
          ...
          <div className="mt-[.2rem]" onClick={(event) => HandleDelete(event.target.parentElement.parentElement)}>
