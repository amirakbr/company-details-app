function LeftSide({companylist , HandleEdit , HandleDelete}) {
     let listShow = companylist.map((index)=> index.Name.length >= 1 ? <tr id={companylist.indexOf(index)} className="border-t-2 border-b-2">
     ...
     <td className="flex gap-6 items-center justify-end mt-[0.025rem] px-2 py-3 pr-8 w-[9rem]"> 
          ...
          <div onClick={(event) => HandleDelete(event.target.parentElement.parentElement)}>
